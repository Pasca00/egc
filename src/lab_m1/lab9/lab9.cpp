...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["grass"], mapTextures["bamboo"]);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1i(glGetUniformLocation(shader->program, "mix_required"), (texture1 != NULL && texture2 != NULL));
    glUniform1f(glGetUniformLocation(shader->program, "time"), Engine::GetElapsedTime());

    if (mesh == meshes["sphere"]) {
        glUniform1i(glGetUniformLocation(shader->program, "is_globe"), 1);
    } else {
        glUniform1i(glGetUniformLocation(shader->program, "is_globe"), 0);
    }
