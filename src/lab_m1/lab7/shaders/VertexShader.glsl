...
void main()
{
    ...
    float ambient_light = 0.25 * material_ks;
    ...
    if (diffuse_light > 0) {
        ...
        //specular_light = material_ks * intensity * max(dot(L, world_normal), 0) * pow(max(dot(V, R), 0), material_shininess);
    } else {
        specular_light = 0;
    }

    float attenuation_factor = 1 / pow(distance(light_position, world_pos), 2);
